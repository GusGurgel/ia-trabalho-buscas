...
class Node_A_Star:
    ...
    def get_neighbors(self):
        costs = []
        neighbors = []
        if Node_A_Star.cost_function == "c1":
            # Todas tem custo 10
            costs = [10] * 4
        elif Node_A_Star.cost_function == "c2":
            # f1, f2 tem custo 15
            # f3, f4 tem custo 10
            costs = [15] * 2 + [10] * 2
        elif Node_A_Star.cost_function == "c3":
            # t = profundidade do nó
            # f1, f2 tem custo 10 + (|5-t| mod 6)
            # f3, f4 tem custo 10
            costs =  [10 + (abs(5 - (self.depth + 1)) % 6)] * 2 + [10] * 2
        elif Node_A_Star.cost_function == "c4":
            # f1, f2 tem custo 5 + (|10-t| mod 11)
            # f3, f4 tem custo 10
            costs =  [5 + (abs(10 - (self.depth + 1)) % 11)] * 2 + [10] * 2
        else:
            raise Exception(f"Invalid cost function -> {Node_A_Star.cost_function}")
        
        for i in range(len(ACTIONS)):
            neighbor_pos = ACTIONS[i]((self.state[0], self.state[1]))
            went_through_pharmacy = True if neighbor_pos in Node_A_Star.pharmacy_positions else self.state[2]
            neighbor_state = (*neighbor_pos, went_through_pharmacy)
            neighbor_cost = costs[i]
            neighbor = Node_A_Star(neighbor_state, neighbor_cost, self)
    ...
    def __gt__(self, other):
        if Node_A_Star.use_a_star_compration and Node_A_Star.a_start_objective_node != None:
            self_cost = self.accumulate_cost + self.get_heuristic_value(Node_A_Star.a_start_objective_node)
            other_cost = other.accumulate_cost + other.get_heuristic_value(Node_A_Star.a_start_objective_node)
